...
let gattahuCurrentLevel = 15 * 2; // 초기 레벨 2배
let gattahuCurrentStage = 1;
let gattahuCurrentBetUnit = 0;
let gattahuTotalUnits = 15 * 2; // 시작 유닛 2배
...
function gattahuInitializeGame() {
    gattahuCurrentLevel = 15 * 2; // 시작 레벨 2배
    gattahuCurrentStage = 1;
    gattahuTotalUnits = 15 * 2; // 시작 유닛 2배
...
function gattahuUpdateDisplay() {
    ...
    // 게임 승리 조건 확인
    if (gattahuTotalUnits >= 30 * 2) { // 승리 유닛 2배
        gattahuGameWin("축하합니다! 총 유닛이 60에 도달하여 게임에 승리했습니다!");
        return; // 승리 시 추가 로직 실행 방지
    }
    // 게임 패배 조건 확인 (초기화 시점 제외)
    // totalUnits가 0 이하가 되면 게임 오버. 단, 초기 totalUnits가 30일 때는 예외.
    if (gattahuTotalUnits <= 0 && !(gattahuCurrentLevel === 30 * 2 && gattahuCurrentStage === 1 && gattahuTotalUnits === 30 * 2)) { // 초기 유닛도 2배로 고려
...
function gattahuHandleLose() {
    ...
    // 레벨 44-52 (기존 22-26)의 4단계 특수 패배 로직
        ...
        if (unitsLost === (stageData.bet1)) { // bet1도 2배로 계산된 값과 비교
            nextLevel = gattahuCurrentLevel - (15 * 2); // 15 * 2 레벨 감소
